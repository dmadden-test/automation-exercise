...
def step_impl(context):
    ...
    context.page.get_by_role("textbox", name="First name *").fill(context.new_user.get("firstname"))
    context.page.get_by_role("textbox", name="Last name *").fill(context.new_user.get("lastname"))
    context.page.get_by_role("textbox", name="Company", exact=True).fill(context.new_user.get("Company"))
    context.page.get_by_role("textbox", name="Address * (Street address, P.").fill(context.new_user.get("address1"))
    context.page.get_by_role("textbox", name="Address 2").fill(context.new_user.get("address2"))
    context.page.get_by_label("Country *").select_option("New Zealand")
    context.page.get_by_role("textbox", name="State *").fill(context.new_user.get("state"))
    context.page.get_by_role("textbox", name="City *").fill(context.new_user.get("city"))
    context.page.locator("#zipcode").fill(context.new_user.get("zipcode"))
    context.page.get_by_role("textbox", name="Mobile Number *").fill(context.new_user.get("mobile_number"))
...
def step_impl(context):
    username = context.new_user.get("name")
    expect(context.page.get_by_text(f'Logged in as {username}', exact=True)).to_be_visible()
